...
def load_trade_limits():
    """
    Loads trade limits configuration from JSON.
    """
    global trade_limits_cache

    if trade_limits_cache is not None:
        logger.info("Trade limits loaded from cache.")
        return trade_limits_cache

    if not os.path.exists(TRADE_LIMIT_FILE):
        logger.warning("Trade limits file not found.")
        # Is this a bug> Shouldn't we generate default limits here???
        trade_limits_cache = {}
        return trade_limits_cache
    try:
...
def get_limit_clearance(symbol):
    ...
    """
    trade_limits = load_limits()
    if symbol not in trade_limits:
        logger.warning(f"No Clearance. Symbol {symbol} not found in trade limits.")
        return None, None
